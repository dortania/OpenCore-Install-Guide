...
Settings relating to boot.efi patching and firmware fixes, for us we care about enabling `RebuildAppleMemoryMap` and disabling `EnableWriteUnprotector`.
...
* **SetupVirtualMap**: YES
  * Fixes SetVirtualAddresses calls to virtual addresses, not enabling may cause a kernel panic depending on your hardware
...
By default, the Sample.plist has this section set for iGPU and Audio. We have no iGPU so PciRoot `PciRoot(0x0)/Pci(0x2,0x0)` can be removed from `Add` section. For audio we'll be using AppleALC or VoodooHDA, so removal of `PciRoot(0x0)/Pci(0x1b,0x0)` is also recommended from both `Add` and `Block` sections
