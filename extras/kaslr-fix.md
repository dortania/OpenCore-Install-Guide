...
* Enable much needed BIOS settings:
  * `Above4GDecoding`: This allows devices to use memory regions above 4GB meaning macOS will have more room to fit, can be problematic on some X99 / X299 if BIOS `MMIOHBase` not set to ``12T`` or below due to macOS only supporting 44-bit physical addressing.
