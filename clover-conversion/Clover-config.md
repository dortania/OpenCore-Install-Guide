...
* GFX patches: Handled by WhateverGreen
   ...
   * change PEGP to GFX0
   * change SL01 to PEGP
* EC Patches: See here on best solution: [Getting started with ACPI](https://acpi.dortania.ml/)
   * change EC0 to EC
   * change H_EC to EC
   * change ECDV to EC
   * change PGEC to EC
* Audio renames: Handled by AppleALC
   ...
   * change CAVS to HDEF
   * change AZAL to HDEF
* Z390 BIOS RTC bug fix: See here on best solution: [Getting started with ACPI](https://acpi.dortania.ml/)(SSDT-AWAC)
...
* **FixShutdown**:
   * [FixShutdown-USB-SSDT](https://github.com/dortania/Opencore-Desktop-Guide/blob//master/extra-files/FixShutdown-USB-SSDT.dsl)
...
* **AddPNLF**:
   * See [SSDT-PNLF](https://acpi.dortania.ml/Laptops/backlight.html)
...
* **PluginType**:
   ...
   * See [Getting started with ACPI](https://acpi.dortania.ml/Universal/plug.html) for more details
...
**Note**: See here on making an SSDT for GPU Spoofing, DeviceProperties injection via OpenCore seems to fail sometimes when trying to spoof a GPU: [Renaming GPUs](https://acpi.dortania.ml/Universal/spoof.html)
